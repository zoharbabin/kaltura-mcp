...
DEFAULT_OPERATIONS = ["list_media", "get_media", "list_categories", "get_category", "list_users", "get_user"]
    ...
    async def run_tests(self, operations: List[str], strategies: List[str], num_requests: int, page_size: int) -> Dict[str, Any]:
        ...
        for operation in operations:
            ...
            for strategy in strategies:
                ...
                print(f"  Average: {avg_time:.4f}s, Min: {min_time:.4f}s, Max: {max_time:.4f}s, Median: {median_time:.4f}s, StdDev: {stdev_time:.4f}s")
...
async def main():
    ...
    parser.add_argument("--page-size", type=int, default=DEFAULT_PAGE_SIZE, help=f"Page size for list operations (default: {DEFAULT_PAGE_SIZE})")
    parser.add_argument("--num-requests", type=int, default=DEFAULT_NUM_REQUESTS, help=f"Number of requests per test (default: {DEFAULT_NUM_REQUESTS})")
    parser.add_argument("--strategies", type=str, default=",".join(DEFAULT_STRATEGIES), help=f"Comma-separated list of strategies to test (default: {','.join(DEFAULT_STRATEGIES)})")
    parser.add_argument("--operations", type=str, default=",".join(DEFAULT_OPERATIONS), help=f"Comma-separated list of operations to test (default: {','.join(DEFAULT_OPERATIONS)})")
