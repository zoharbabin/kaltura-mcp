    ...
    async def handle(self, uri: str) -> List[types.ResourceContents]:
        ...
        try:
            ...
            types.ResourceContents(
                uri=uri,
                mimeType="application/json",
                text=json.dumps({"error": f"Error getting category: {str(e)}"}, indent=2, cls=KalturaJSONEncoder)
            )
        ]
    ...
    async def handle(self, uri: str) -> List[types.ResourceContents]:
        ...
        try:
            result = await self.kaltura_client.execute_request(
                "category", "list",
                filter=category_filter, pager=pager
            )
            ...
            raise ValueError(f"Error listing categories: {str(e)}")
