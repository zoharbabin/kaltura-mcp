...
class EnhancedMediaUploadToolHandler(KalturaToolHandler):
                    ...
                    try:
                        ...
                        for retry in range(max_retries):
                            try:
                                ...
                                verification = await self.kaltura_client.execute_request(
                                    "data", "get",
                                    entryId=entry_id
                                )
                                if verification and hasattr(verification, 'id'):
                                    ...
                                    raise ValueError(f"Data entry verification returned invalid result")
                            except Exception as e:
    ...
    async def _create_media_entry(
        ...
        # Add the entry
        try:
            media_entry = await self.kaltura_client.execute_request(
                "media", "add",
                entry=entry
            )
    ...
    async def _create_document_entry(
        ...
        # Add the entry
        try:
            document_entry = await self.kaltura_client.execute_request(
                "document", "add",
                document=entry
            )
