...
class KalturaOAuthManager:
    ...
    def __init__(self):
        ...
        self.auth_url = "https://www.kaltura.com/api_v3/index.php"  # Kaltura doesn't use standard OAuth
...
async def oauth_callback(code: str, state: str):
    ...
    try:
        ...
        # Store user session
        user_id = jwt.decode(token_data["access_token"], JWT_SECRET_KEY, algorithms=[JWT_ALGORITHM])["sub"]
...
async def handle_mcp_message(request: Request, user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        logger.info(f"Received MCP message from user {user['sub']}: {message.get('method', 'unknown')}")
...
async def process_mcp_message(message: Dict[str, Any], kaltura_manager: KalturaClientManager) -> Dict[str, Any]:
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "sort_order": {"type": "string", "enum": ["desc", "asc"], "description": "Sort direction"}
                },
                "required": ["query"],
            },
        ),
...
async def call_tool(tool_name: str, arguments: Dict[str, Any], kaltura_manager: KalturaClientManager) -> List[types.TextContent]:
    """Execute a tool call."""
    try:
