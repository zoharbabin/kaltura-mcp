...
def test_parse_config_data():
    ...
    config_data = {
        ...
        "server": {"log_level": "DEBUG", "transport": "stdio", "port": 8000},
    }
...
def test_apply_env_overrides():
    ...
    config = Config(
        ...
        server=ServerConfig(log_level="INFO", transport="websocket", port=9000, host="localhost", debug=False),
        logging=LoggingConfig(level="INFO", file=None),
        context=ContextConfig(default_strategy="selective", max_entries=50, max_context_size=5000),
    )
...
def test_validate_config_missing_admin_secret():
    ...
    config = Config(
        kaltura=KalturaConfig(partner_id=123, admin_secret="", user_id="test_user", service_url="https://example.com"),
        server=ServerConfig(),
    )
