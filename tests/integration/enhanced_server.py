...
class EnhancedKalturaMcpServer(KalturaMcpServer):
    ...
    async def initialize(self):
        ...
        logger.info(
            f"Enhanced MCP server initialized with {len(self.tool_handlers)} tools and {len(self.resource_handlers)} resources"
        )
    ...
    async def run(self):
        ...
        if self.config.server.transport == "stdio":
            ...
            logger.info(
                f"Starting enhanced MCP server with HTTP transport on {self.config.server.host}:{self.config.server.port}"
            )
            await http_server(
                self.app,
