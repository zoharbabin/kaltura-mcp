...
class TestFullMcpFlow:
    ...
    async def test_category_flow(self, mcp_client):
        ...
        add_response = await mcp_client.post(
            "/tools/kaltura.category.add",
            json={"name": category_name, "description": "Created by MCP integration test"},
        )
        ...
        try:
            ...
            get_response = await mcp_client.post("/tools/kaltura.category.get", json={"id": category_id})
            ...
            update_response = await mcp_client.post(
                "/tools/kaltura.category.update", json={"id": category_id, "name": updated_name}
            )
            ...
            delete_response = await mcp_client.post("/tools/kaltura.category.delete", json={"id": category_id})
    ...
    async def test_resource_access(self, mcp_client):
        ...
        list_response = await mcp_client.post("/tools/kaltura.media.list", json={"page_size": 1})
