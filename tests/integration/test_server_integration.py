...
class TestServerIntegration:
    ...
    async def test_cross_component_workflow(self, server):
        ...
        add_category_result = await server.app.call_tool("kaltura.category.add", {
            "name": category_name,
        ...
        try:
            ...
            try:
                ...
                upload_result = await server.app.call_tool("kaltura.media.upload", {
                ...
                if not entry_id:
                    pytest.skip(f"Media upload returned empty entry ID")

                # 4. Verify the media entry is in the category
                get_category_result = await server.app.call_tool("kaltura.category.get", {
                    "id": category_id
                })
                ...
                for retry in range(max_retries):
                    try:
                        get_result = await server.app.call_tool("kaltura.media.get", {
                            "entry_id": entry_id
                        })
                ...
                if get_result is None and temp_file_path.endswith(".txt"):
                    ...
                    for retry in range(max_retries):
                        try:
                            ...
                            data_entry = await server.kaltura_client.execute_request(
                                "data", "get",
                                entryId=entry_id
                            )
                            ...
                            if retry < max_retries - 1:  # Don't sleep on the last iteration
                                ...
                                pytest.skip(f"Failed to get entry with both media.get and data.get after {max_retries} retries each")
                ...
                assert get_content["name"] == f"Media in Category {unique_id}", f"Expected name to be 'Media in Category {unique_id}' but got: {get_content['name']}"
                ...
                await server.app.call_tool("kaltura.media.delete", {
                    "entry_id": entry_id
                })
